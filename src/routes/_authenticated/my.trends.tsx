...
const DUAL_AXIS_MARGIN = { top: 8, right: 16, left: 8, bottom: 8 };
const Y_AXIS_WIDTH = 44;

/** "2026-06" → "6월" / 연도 바뀌거나 1월이면 "26년 6월" */
function makeTickFmt(monthKeys: string[]) {
  const years = new Set(monthKeys.map((k) => k.slice(0, 4)));
  const multiYear = years.size > 1;
  return (v: string) => {
    const [y, m] = v.split("-");
    const mn = Number(m);
    if (multiYear || mn === 1) return `${y.slice(2)}년 ${mn}월`;
    return `${mn}월`;
  };
}
...
function periodCaption(monthKeys: string[]) {
  if (monthKeys.length === 0) return "데이터 없음";
  if (monthKeys.length === 1) return `${formatMonthLabel(monthKeys[0])} · 1개월`;
  const first = monthKeys[0];
  const last = monthKeys[monthKeys.length - 1];
  const [fy, fm] = first.split("-");
  const [ly, lm] = last.split("-");
  const sameYear = fy === ly;
  const from = sameYear ? `${Number(fm)}월` : `${fy.slice(2)}년 ${Number(fm)}월`;
  const to = sameYear ? `${Number(lm)}월` : `${ly.slice(2)}년 ${Number(lm)}월`;
  return `${from}–${to} · ${monthKeys.length}개월`;
}
...
export function MyTrendsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["my-trends-ext-v6", ownerId ?? "all"],
  ...
  const tickFmt = makeTickFmt(monthKeys);
  const periodText = periodCaption(monthKeys);
  const fewPoints = months.length <= 3;
  const xAxisProps = {
    ...
    interval: 0 as const,
    minTickGap: 8,
    padding: fewPoints ? { left: 28, right: 28 } : undefined,
  };
    ...
    <div className="space-y-6">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/5 bg-[oklch(0.18_0.02_265)] text-white shadow-[0_30px_80px_-30px_oklch(0.45_0.18_265/0.45)] p-5 md:p-6">
        ...
        <div className="relative space-y-3">
          <div className="flex items-center gap-2 text-[11px] text-white/60">
            ...
            <span className="truncate">{s?.insight ?? periodText}</span>
          </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <p className="text-xs text-muted-foreground">{periodText} · 평균 {s?.avgRentalRate ?? 0}%</p>
        </CardHeader>
        <CardContent className="h-64">
          {isLoading ? <Loading /> : months.length === 0 ? <Empty /> : (
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <p className="text-xs text-muted-foreground">{periodText} · 재고 + 회수대상 합산 비율</p>
        </CardHeader>
        <CardContent className="h-64">
          {isLoading ? <Loading /> : months.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height="100%">
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <p className="text-xs text-muted-foreground">{periodText} · 월별 신규 · 누적 거래처 수</p>
        </CardHeader>
...
function Empty() {
  return <div className="h-full grid place-items-center text-muted-foreground text-sm">표시할 월별 데이터가 없습니다.</div>;
}
